...
export default function DashboardSociete() {
  ...
  const [expFilter, setExpFilter] = useState<readonly [number, number] | null>(null);
  ...
  const filteredTalents = talents?.filter((t) => {
    ...
    const matchExp = !expFilter || (
      (t.experience_years ?? 0) >= expFilter[0] && (t.experience_years ?? 0) <= expFilter[1]
    );
    const matchCountry = !countryFilter || (t.country?.toLowerCase().includes(countryFilter.toLowerCase()));
    return matchSearch && matchTension && matchSector && matchExp && matchCountry;
  }) ?? [];
  ...
  const stats = [
    { label: "Talents disponibles", sublabel: "aujourd'hui", value: talentCount, icon: Users, accent: "text-accent" },
    { label: "Candidatures reçues", sublabel: "cette semaine", value: candidatureCount, icon: ClipboardList, accent: "text-primary" },
    { label: "Coût ALTIS moyen", sublabel: "par placement", value: "2 450 €", icon: DollarSign, accent: "text-accent" },
    { label: "Taux matching", sublabel: "Score IA moyen", value: `${avgScore} %`, icon: TrendingUp, accent: "text-primary" },
  ];
    ...
    <DashboardLayout sidebarVariant="entreprise">
      ...
      >
        ...
        >
          ...
          <div className="absolute bottom-0 right-0 w-48 h-48 bg-[radial-gradient(circle,hsl(var(--primary)/0.08),transparent_70%)]" />
        ...
        {/* ══════════════ STATS ══════════════ */}
        <motion.div custom={1} variants={fadeUp} className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
          {stats.map((s, i) => (
            <Card key={i} className="group bg-gradient-to-br from-[hsl(222,33%,14%)] to-[hsl(222,33%,10%)] border-white/10 hover:border-accent/30 transition-all duration-300 hover:shadow-lg hover:shadow-accent/5">
              <CardContent className="p-4 sm:p-5 flex items-center gap-3">
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-accent/15 group-hover:bg-accent/25 transition-colors shrink-0">
        ...
        <motion.div custom={1.5} variants={fadeUp} className="space-y-3">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-11 h-12 sm:h-13 text-sm sm:text-base bg-card/80 border-border/50 rounded-xl focus:border-accent/50 focus:ring-accent/20"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            {activeFiltersCount > 0 && (
              <button
                onClick={() => { setSearch(""); setTensionFilter("Tous"); setSectorFilter(null); setExpFilter(null); setCountryFilter(null); }}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-accent hover:text-accent/80 font-semibold px-2 py-1 rounded-lg bg-accent/10"
              >
          ...
          {/* Tension + Sector + Experience filters */}
          <div className="flex flex-wrap gap-2">
            ...
            <span className="w-px h-6 bg-border/40 self-center mx-0.5" />
            ...
            {EXP_FILTERS.map((f) => (
              <button
                key={f.label}
                onClick={() => setExpFilter(expFilter === f.value ? null : (f.value as readonly [number, number] | null))}
        ...
        <motion.div custom={2} variants={fadeUp}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
              ...
              <Badge variant="secondary" className="ml-1 text-xs tabular-nums">{filteredTalents.length}</Badge>
            </h2>
            <Badge className="bg-accent/15 text-accent border-0 text-[10px] font-bold px-2.5 gap-1 hidden sm:flex">
              ...
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1 ${
                  countryFilter === cf.value
                    ...
                    : "bg-card border border-border/40 text-muted-foreground hover:border-accent/30 hover:text-foreground"
                }`}
              >
          ...
          {filteredTalents.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredTalents.slice(0, 9).map((t, i) => (
                <motion.div
                  key={t.id}
                      ...
                      <div className="p-4 sm:p-5 space-y-3">
                        ...
                        <div className="flex flex-col gap-2 pt-1">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            className="w-full gap-1.5 text-xs text-muted-foreground hover:text-foreground h-8"
            ...
            <Card className="bg-card border-border/50">
              <CardContent className="p-10 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {search || sectorFilter || expFilter ? "Aucun talent ne correspond à vos critères." : "Aucun talent disponible pour le moment."}
                </p>
        ...
        <motion.div custom={3} variants={fadeUp}>
          <h2 className="font-display text-lg sm:text-xl font-bold text-foreground mb-4 flex items-center gap-2">
            ...
            Candidatures reçues
            <Badge variant="secondary" className="ml-1 text-xs tabular-nums">{candidatures?.length ?? 0}</Badge>
          </h2>
                  ...
                  <Card className="group bg-card border-border/50 hover:border-accent/40 hover:shadow-lg hover:shadow-accent/5 transition-all duration-300">
                    <CardContent className="p-5 space-y-3">
                      <div className="flex items-start gap-3">
                        <div className="h-11 w-11 rounded-full bg-gradient-to-br from-accent/30 to-primary/30 flex items-center justify-center text-white font-bold text-lg shrink-0">
                          {(c.full_name || "T")[0].toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-foreground text-sm truncate">{c.full_name || "Talent"}</p>
                      ...
                      <div className="flex flex-col gap-2">
                        <Button size="sm" variant="outline" className="w-full gap-1.5 text-xs border-accent/30 text-accent hover:bg-accent/10 h-9">
                          <Eye className="h-3.5 w-3.5" /> Voir le profil complet
                        </Button>
                        <Button
                          size="sm"
                          className="w-full gap-1.5 text-xs bg-gradient-to-r from-accent to-primary text-white hover:opacity-90 h-9"
                          disabled={altisLoading === c.id}
                          onClick={() => handleAltis(c.full_name || "ce talent", c.id)}
                        >
                          <Sparkles className="h-3.5 w-3.5" /> {altisLoading === c.id ? "Redirection…" : "Activer ALTIS (2 450 €)"}
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <Accordion type="single" collapsible className="w-full">
                  {candidatures.map((c) => {
                    const statusMap: Record<string, { label: string; color: string }> = {
                      ...
                      in_review: { label: "En analyse", color: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30" },
                    ...
                    return (
                      <AccordionItem key={c.id} value={c.id} className="border-border/30">
                        <AccordionTrigger className="px-5 py-3 hover:no-underline hover:bg-muted/30 transition-colors">
                          <div className="flex items-center gap-3 w-full pr-4">
                            <div className="h-8 w-8 rounded-full bg-gradient-to-br from-accent/30 to-primary/30 flex items-center justify-center text-white font-bold text-sm shrink-0">
                              {(c.full_name || "T")[0].toUpperCase()}
                            </div>
                            ...
                            <Badge className={`text-[10px] border font-semibold px-2 py-0.5 shrink-0 ${st.color}`}>
                              <CircleDot className="h-2.5 w-2.5 mr-1" />{st.label}
                            </Badge>
                          </div>
                        </AccordionTrigger>
        ...
        <motion.div custom={4} variants={fadeUp}>
          <h2 className="font-display text-lg sm:text-xl font-bold text-foreground mb-5 flex items-center gap-2">
                    ...
                    <CardContent className="p-4 sm:p-5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="h-7 text-xs text-accent gap-1 hover:bg-accent/10"
                          onClick={() => handleInvite({ user_id: t.user_id, full_name: t.full_name })}
                          disabled={invitingId === t.user_id}
                        >
                          {invitingId === t.user_id ? "Envoi…" : <>Contacter <ChevronRight className="h-3 w-3" /></>}
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
