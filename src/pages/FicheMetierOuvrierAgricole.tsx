...
import {
  Leaf, Briefcase, ShieldCheck, ArrowRight, CheckCircle2,
  MapPin, Euro, Clock, ThermometerSun, Plane, Home, Globe,
  FileCheck, AlertTriangle, ArrowLeft, Mail, Sun, Sprout
} from "lucide-react";
...
import heroImg from "@/assets/metier-ouvrier-agricole.jpg?format=webp";
...
const MISSIONS = [
  "Préparer les sols (labour, amendement, semis) selon les cultures",
  "Planter, entretenir et récolter les cultures maraîchères, viticoles ou céréalières",
  "Assurer l'arrosage, le désherbage et les traitements phytosanitaires",
  "Conduire et entretenir le matériel agricole (tracteur, moissonneuse)",
  "Trier, conditionner et stocker les récoltes",
  "Participer à l'élevage et aux soins des animaux si applicable",
  "Respecter les normes d'hygiène, de sécurité et les certifications bio/HVE",
];
const COMPETENCES = [
  { icon: Leaf, text: "Techniques culturales et maraîchères" },
  { icon: ShieldCheck, text: "Normes phytosanitaires et Certiphyto" },
  { icon: Sun, text: "Conduite d'engins agricoles (tracteur)" },
  { icon: Sprout, text: "Agriculture biologique et raisonnée" },
];
const CONDITIONS = [
  "Travail en plein air, soumis aux conditions climatiques",
  "Horaires saisonniers, avec pics d'activité pendant les récoltes",
  "Métier physique nécessitant endurance et résistance",
  "Forte demande saisonnière et possibilité de CDI dans les grandes exploitations",
];
...
export default function FicheMetierOuvrierAgricole() {
  return (
    <>
      <Helmet>
        <title>Ouvrier agricole polyvalent – Code ROME A1101 | AXIOM Talents</title>
        <meta name="description" content="Fiche métier Ouvrier agricole polyvalent (ROME A1101) : missions, compétences, salaire en France et accompagnement AXIOM & ALTIS." />
        <link rel="canonical" href="https://axiom-talents.com/fiches-metiers/a1101-ouvrier-agricole" />
        <script type="application/ld+json">{JSON.stringify({
          "@context": "https://schema.org", "@type": "JobPosting",
          "title": "Ouvrier agricole polyvalent", "description": "Travaux agricoles polyvalents : préparation des sols, plantation, récolte. Métier en haute tension en France.",
          "identifier": { "@type": "PropertyValue", "name": "ROME", "value": "A1101" },
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        <motion.section initial="hidden" animate="visible" className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-6xl px-4 py-20 md:py-28">
            ...
            <motion.h1 custom={1} variants={fadeUp} className="font-display text-3xl font-bold md:text-5xl">Ouvrier agricole polyvalent</motion.h1>
            <motion.p custom={2} variants={fadeUp} className="mt-2 text-muted-foreground text-lg">Code ROME A1101 · Agriculture</motion.p>
            <motion.div custom={3} variants={fadeUp} className="mt-6 flex items-center gap-6 flex-wrap">
              <span className="flex items-center gap-2 text-success font-semibold"><Euro className="h-5 w-5" /> 1 700 – 2 300 € net/mois</span>
              <span className="flex items-center gap-2 text-muted-foreground"><MapPin className="h-4 w-4" /> France entière</span>
              <span className="flex items-center gap-2 text-muted-foreground"><Clock className="h-4 w-4" /> Temps plein / Saisonnier</span>
            </motion.div>
          </div>
        </motion.section>
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
            ...
            <Separator />
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }}><motion.h2 custom={0} variants={fadeUp} className="flex items-center gap-2 text-xl font-bold"><Leaf className="h-5 w-5 text-accent" /> Compétences recherchées</motion.h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{COMPETENCES.map((c, i) => (<motion.div key={i} custom={i + 1} variants={fadeUp}><Card className="border-accent/20 bg-accent/5"><CardContent className="flex items-center gap-3 p-4"><c.icon className="h-5 w-5 text-accent shrink-0" /><span className="text-sm font-medium">{c.text}</span></CardContent></Card></motion.div>))}</div></motion.div>
            <Separator />
          ...
          <div className="space-y-6">
            <Card className="border-accent/30 bg-accent/5"><CardContent className="p-6 space-y-4">
              ...
              <Button asChild className="w-full bg-accent text-accent-foreground hover:bg-accent/90"><Link to="/signup-light?rome=A1101">Vérifier mon éligibilité et commencer mon parcours <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </CardContent></Card>
            <Card><CardContent className="p-6 space-y-3"><h3 className="font-bold">En résumé</h3><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-accent text-lg">A1101</p><p className="text-muted-foreground text-xs">Code ROME</p></div><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-success text-lg">~1 900 €</p><p className="text-muted-foreground text-xs">Salaire moyen</p></div></div></CardContent></Card>
          </div>
        </div>
