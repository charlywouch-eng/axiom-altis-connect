...
import { Lock, Copy, Check, Download, Share2, RotateCcw, CheckCircle2, Users, MessageSquare, AlertTriangle, Star } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { trackGA4 } from "@/lib/ga4";
import {
  BAND_LABELS, BAND_SCALE, DISCLAIMER, FREIN_COPY, METHODOLOGY, VERDICT, getResult, reportReference, sendTestimonial,
...
export default function SimulationResultat() {
  ...
  return (
    <SimShell title="Votre résultat — Test d'éligibilité AXIOM">
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        {error && (
          <div className="ax-card p-8 text-center">
            <p style={{ font: `15px ${T.sans}`, marginBottom: 16 }}>{error}</p>
            <Link to="/leads" className="ax-btn">Faire le test</Link>
          </div>
        )}
        {!error && !r && <p style={{ font: `14px ${T.sans}`, color: T.muted }}>Chargement de votre résultat…</p>}

        {r && band && !r.unlocked && (
          <div className="grid gap-5 ax-in">
            {/* ── Verdict gratuit ── */}
            <section className="ax-card p-6 sm:p-8">
              <p className="ax-eyebrow">Votre résultat · {r.metier.titre}</p>
              <h1 style={{ font: `600 clamp(26px,5vw,36px)/1.15 ${T.serif}`, margin: "10px 0 12px" }}>
                {r.first_name}, {VERDICT[r.band].title.charAt(0).toLowerCase() + VERDICT[r.band].title.slice(1)}
              </h1>
              <p style={{ font: `16px/1.6 ${T.sans}`, color: T.ink2, margin: 0 }}>{VERDICT[r.band].text}</p>
              <div className="flex flex-wrap gap-2 mt-5">
                <span style={{ font: `600 13px ${T.sans}`, color: band.color, background: band.soft, borderRadius: 6, padding: "6px 11px" }}>{band.label}</span>
                <span className="ax-num" style={{ font: `600 13px ${T.sans}`, color: T.ink2, background: T.bg, border: `1px solid ${T.line}`, borderRadius: 6, padding: "5px 11px" }}>
                  {r.criteres_ok} critères sur {r.criteres_total} au niveau attendu
                </span>
                <span style={{ font: `600 13px ${T.sans}`, color: T.ink2, background: T.bg, border: `1px solid ${T.line}`, borderRadius: 6, padding: "5px 11px" }}>
                  Demande pour votre métier : {r.metier.tension.toLowerCase()}
                </span>
              </div>
            </section>

            {/* ── Point d'attention n°1 (gratuit) ── */}
            {r.frein && FREIN_COPY[r.frein.cle] && (
              <section className="ax-card p-6 sm:p-7 flex gap-4" style={{ borderLeft: "4px solid #A15C00" }}>
                <AlertTriangle className="h-6 w-6 shrink-0 mt-0.5" style={{ color: "#A15C00" }} />
                <div>
                  <p className="ax-eyebrow" style={{ color: "#A15C00" }}>Votre point d'attention n°1</p>
                  <p style={{ font: `600 19px/1.3 ${T.sans}`, margin: "6px 0 6px", color: T.ink }}>{FREIN_COPY[r.frein.cle].titre}</p>
                  <p style={{ font: `15px/1.55 ${T.sans}`, color: T.ink2, margin: 0 }}>{FREIN_COPY[r.frein.cle].pourquoi}</p>
                  <p style={{ font: `600 15px/1.5 ${T.sans}`, color: T.ink, margin: "12px 0 0" }}>Bonne nouvelle : ce point se travaille. Votre rapport complet vous dit comment, étape par étape.</p>
                </div>
              </section>
            )}

            {/* ── Rapport complet réservé ── */}
            <section className="ax-card overflow-hidden">
              <div className="p-6 sm:p-7" style={{ background: "#0B1F3A", color: "#fff" }}>
                <p style={{ font: `600 11px ${T.sans}`, letterSpacing: ".14em", color: "#C9D6EA", margin: 0 }}>COHORTE FONDATRICE 2026</p>
                <h2 style={{ font: `600 clamp(21px,4vw,26px)/1.25 ${T.serif}`, color: "#fff", margin: "8px 0 10px" }}>Votre rapport complet vous attend.</h2>
                <ul className="grid gap-2 sm:grid-cols-2" style={{ listStyle: "none", padding: 0, margin: 0, font: `14px/1.45 ${T.sans}`, color: "#DCE5F2" }}>
                  {["Votre indice exact sur 100", "Vos points sur chacun des 5 critères", "Comment lever votre point d'attention", "Votre plan d'action et le PDF à présenter"].map(i => (
                    <li key={i} className="flex gap-2 items-start"><Lock className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "#8FB3F5" }} />{i}</li>
                  ))}
                </ul>
                <p style={{ font: `14px/1.55 ${T.sans}`, color: "#C9D6EA", margin: "16px 0 0" }}>
                  Rejoignez la cohorte fondatrice : les premiers candidats de la bêta seront prioritaires lors des premières mises en relation avec les employeurs.
                </p>
              </div>

              <div role="tablist" className="flex" style={{ borderBottom: `1px solid ${T.line}` }}>
                {([["share", Users, "Inviter 10 proches"], ["review", MessageSquare, "Donner mon avis"]] as const).map(([k, Icon, label]) => (
              ...
              <div className="p-6 sm:p-7">
                {tab === "share" ? (
                  <div className="grid gap-4">
                    <p style={{ font: `15px/1.55 ${T.sans}`, color: T.ink2, margin: 0 }}>
                      Vous connaissez d'autres personnes qui visent un emploi en France ? Envoyez-leur le test. Dès que <strong>{r.threshold} d'entre elles</strong> l'ont ouvert, votre rapport complet s'affiche ici.
                    </p>
                    <div>
                      <div className="flex justify-between" style={{ font: `14px ${T.sans}`, marginBottom: 8 }}>
                        <span style={{ fontWeight: 600 }}>Invitations ouvertes</span>
                      ...
                      <p style={{ font: `12.5px ${T.sans}`, color: T.muted, margin: "8px 0 0" }}>Chaque personne n'est comptée qu'une fois ; cette page se met à jour toute seule.</p>
                    </div>
                    <ShareActions code={r.referral_code} onShare={canal => trackGA4("sim_share_clicked", { canal })} />
                  </div>
                ) : (
                  <div className="grid gap-3">
                    <label className="ax-label" htmlFor="sim-temoignage">Qu'avez-vous pensé de ce test ?</label>
                    <p className="ax-help" style={{ margin: 0 }}>Votre avis nous aide à l'améliorer. Il pourra être publié avec votre prénom et votre pays uniquement. Votre rapport s'affiche dès l'envoi.</p>
                    ...
                    <div className="flex justify-between items-center gap-3 flex-wrap">
                      ...
                      <span className="ax-num" style={{ font: `12px ${T.sans}`, color: testimonial.trim().length >= MIN_TESTIMONIAL ? "#0F7B5F" : T.muted }}>
                        {testimonial.trim().length >= MIN_TESTIMONIAL ? <span className="inline-flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> Prêt à envoyer</span> : `${testimonial.trim().length} / ${MIN_TESTIMONIAL} caractères minimum`}
                      </span>
                    </div>
                    <button type="button" className="ax-btn w-full sm:w-auto" disabled={sending || testimonial.trim().length < MIN_TESTIMONIAL} onClick={submitTestimonial}>
                      {sending ? "Envoi…" : "Envoyer et voir mon rapport complet"}
                    </button>
                  </div>
                )}
              </div>
            ...
            <p style={{ font: `12.5px/1.6 ${T.sans}`, color: T.muted, margin: 0 }}>
              <strong style={{ color: T.ink2 }}>Méthode.</strong> {DISCLAIMER}{" "}
              <a href="/" className="underline underline-offset-2" style={{ color: T.brand }}>Découvrir AXIOM</a>
            </p>
          </div>
        )}

        {r && band && r.unlocked && typeof r.score === "number" && (
          <div className="grid gap-5 ax-in">
            {/* ── En-tête du rapport ── */}
            <section className="ax-card overflow-hidden">
              <div className="px-5 sm:px-7 pt-6 pb-5" style={{ borderBottom: `1px solid ${T.line}` }}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="ax-eyebrow">Rapport d'évaluation complet</p>
                    <h1 style={{ font: `600 clamp(22px,3.4vw,30px)/1.2 ${T.serif}`, margin: "8px 0 0" }}>Éligibilité à l'emploi en France</h1>
                  </div>
                  {r.founder && (
                    <span className="inline-flex items-center gap-1.5" style={{ font: `600 12px ${T.sans}`, color: T.brand, background: T.brandSoft, borderRadius: 999, padding: "6px 12px" }}><Star className="h-3.5 w-3.5" /> Membre fondateur · 2026</span>
                  )}
                </div>
                <dl className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-4 mt-6" style={{ font: `14px ${T.sans}` }}>
                  {[["Candidat", r.first_name], ["Métier évalué", r.metier.titre], ["Pays de résidence", r.pays], ["Date", date]].map(([k, v]) => (
                    <div key={k}><dt style={{ font: `12px ${T.sans}`, color: T.muted, marginBottom: 2 }}>{k}</dt><dd style={{ fontWeight: 600, margin: 0 }}>{v}</dd></div>
                  ))}
                </dl>
              ...
              <div className="px-5 sm:px-7 py-6 grid gap-6 md:grid-cols-[200px_1fr] items-start">
                <div>
                  <p className="ax-eyebrow">Indice de préparation</p>
                  <p className="ax-num" style={{ margin: "6px 0 0", lineHeight: 1 }}>
                    <span style={{ font: `600 60px/1 ${T.serif}`, color: T.ink }}>{r.score}</span>
                ...
                <div>
                  <p style={{ font: `16px/1.6 ${T.sans}`, color: T.ink2, margin: "0 0 18px" }}>{band.summary}</p>
            ...
            <section className="ax-card overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full" style={{ font: `14px ${T.sans}`, borderCollapse: "collapse", minWidth: 480 }}>
                  <thead>
                    <tr style={{ background: T.bg, color: T.muted, font: `600 12px ${T.sans}`, textTransform: "uppercase", letterSpacing: ".06em" }}>
                      <th className="text-left px-5 sm:px-7 py-2.5">Critère</th><th className="text-right px-3 py-2.5">Pondération</th><th className="text-right px-5 sm:px-7 py-2.5">Vos points</th>
                    </tr>
                  </thead>
                  <tbody>
                    {METHODOLOGY.map(c => {
                      const p = points(c.cle) ?? 0;
                      return (
                        <tr key={c.cle} style={{ borderTop: `1px solid ${T.line}` }}>
                          <td className="px-5 sm:px-7 py-3.5"><span style={{ display: "block", fontWeight: 600 }}>{c.label}</span><span style={{ display: "block", font: `12.5px ${T.sans}`, color: T.muted }}>{c.detail}</span></td>
                          ...
                          <td className="px-5 sm:px-7 py-3.5 text-right">
                            <span className="inline-flex items-center gap-3 justify-end">
                              <span style={{ width: 64, height: 6, background: T.line, borderRadius: 3, overflow: "hidden" }} className="hidden sm:block"><span style={{ display: "block", height: "100%", width: `${(p / c.max) * 100}%`, background: T.brand }} /></span>
            ...
            <section className="ax-card p-5 sm:p-7 grid gap-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 style={{ font: `600 18px ${T.sans}`, margin: 0 }}>Votre plan d'action</h2>
              ...
              <div className="pt-5" style={{ borderTop: `1px solid ${T.line}` }}>
                <p style={{ font: `600 14px ${T.sans}`, margin: "0 0 10px" }}>Un proche vise aussi la France ? Envoyez-lui le test.</p>
                <ShareActions code={r.referral_code} onShare={canal => trackGA4("sim_share_clicked", { canal })} />
              </div>
            </section>

            <section className="grid gap-3 sm:grid-cols-[1fr_auto] items-start" style={{ font: `12.5px/1.6 ${T.sans}`, color: T.muted }}>
              <p style={{ margin: 0 }}><strong style={{ color: T.ink2 }}>Méthodologie et limites.</strong> {DISCLAIMER}</p>
              <Link to="/leads" className="inline-flex items-center gap-1.5" style={{ color: T.brand, fontWeight: 500 }}><RotateCcw className="h-3.5 w-3.5" /> Nouveau test</Link>
            </section>
          </div>
        )}
      </main>
