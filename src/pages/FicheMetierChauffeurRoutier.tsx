...
import {
  Truck, Briefcase, ShieldCheck, ArrowRight, CheckCircle2,
  MapPin, Euro, Clock, ThermometerSun, Plane, Home, Globe,
  FileCheck, AlertTriangle, ArrowLeft, Mail, Fuel, GraduationCap
} from "lucide-react";
...
import heroImg from "@/assets/metier-chauffeur-routier.jpg?format=webp";
...
const MISSIONS = [
  "Assurer le transport de marchandises en national et/ou international",
  "Respecter les temps de conduite et de repos réglementaires",
  "Charger et décharger la marchandise en toute sécurité",
  "Contrôler l'état du véhicule avant et après chaque trajet",
  "Remplir les documents de transport (CMR, bons de livraison)",
  "Respecter le code de la route et les réglementations ADR si applicable",
  "Communiquer avec le service logistique et les clients lors des livraisons",
];
const COMPETENCES = [
  { icon: Truck, text: "Permis C/CE et FIMO/FCO à jour" },
  { icon: ShieldCheck, text: "Réglementation transport et temps de conduite" },
  { icon: GraduationCap, text: "Utilisation du chronotachygraphe numérique" },
  { icon: Fuel, text: "Éco-conduite et gestion de carburant" },
];
const CONDITIONS = [
  "Déplacements régionaux, nationaux ou internationaux selon le poste",
  "Horaires variables incluant nuits et week-ends possibles",
  "Autonomie sur la route avec responsabilité du chargement",
  "Forte demande dans toute la France, CDI fréquents",
];
...
export default function FicheMetierChauffeurRoutier() {
  return (
    <>
      <Helmet>
        <title>Chauffeur routier – Code ROME N4101 | AXIOM Talents</title>
        <meta name="description" content="Fiche métier Chauffeur routier (ROME N4101) : missions, compétences, salaire attractif en France et accompagnement AXIOM & ALTIS." />
        <link rel="canonical" href="https://axiom-talents.com/fiches-metiers/n4101-chauffeur-routier" />
        <script type="application/ld+json">{JSON.stringify({
          "@context": "https://schema.org", "@type": "JobPosting",
          "title": "Chauffeur routier", "description": "Transport de marchandises en national et international. Métier en très haute tension en France.",
          "identifier": { "@type": "PropertyValue", "name": "ROME", "value": "N4101" },
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        <motion.section initial="hidden" animate="visible" className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-6xl px-4 py-20 md:py-28">
            ...
            <motion.h1 custom={1} variants={fadeUp} className="font-display text-3xl font-bold md:text-5xl">Chauffeur routier</motion.h1>
            <motion.p custom={2} variants={fadeUp} className="mt-2 text-muted-foreground text-lg">Code ROME N4101 · Transport & Logistique</motion.p>
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
            ...
            <Separator />
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }}><motion.h2 custom={0} variants={fadeUp} className="flex items-center gap-2 text-xl font-bold"><Truck className="h-5 w-5 text-accent" /> Compétences recherchées</motion.h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{COMPETENCES.map((c, i) => (<motion.div key={i} custom={i + 1} variants={fadeUp}><Card className="border-accent/20 bg-accent/5"><CardContent className="flex items-center gap-3 p-4"><c.icon className="h-5 w-5 text-accent shrink-0" /><span className="text-sm font-medium">{c.text}</span></CardContent></Card></motion.div>))}</div></motion.div>
            <Separator />
          ...
          <div className="space-y-6">
            <Card className="border-accent/30 bg-accent/5"><CardContent className="p-6 space-y-4">
              ...
              <Button asChild className="w-full bg-accent text-accent-foreground hover:bg-accent/90"><Link to="/signup-light?rome=N4101">Vérifier mon éligibilité et commencer mon parcours <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </CardContent></Card>
            <Card><CardContent className="p-6 space-y-3"><h3 className="font-bold">En résumé</h3><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-accent text-lg">N4101</p><p className="text-muted-foreground text-xs">Code ROME</p></div><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-success text-lg">~2 600 €</p><p className="text-muted-foreground text-xs">Salaire moyen</p></div></div></CardContent></Card>
          </div>
        </div>
