...
export default function OffreFiche() {
  ...
  // Try loading from job_offers DB first
  ...
  const title = dbOffer?.title ?? "Offre AXIOM";
  const location = dbOffer?.location ?? "France";
  const salary = dbOffer?.salary_range ?? null;
  const description = dbOffer?.description ?? "Détails de l'offre à venir.";
  const skills = dbOffer?.required_skills ?? [];
  ...
  const score = talentProfile?.score ?? 75;
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-accent/30 bg-accent/5">
              <CardHeader><CardTitle className="flex items-center gap-2"><GraduationCap className="h-5 w-5 text-accent" /> Accompagnement ALTIS inclus</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {[
                  ...
                  <div className="relative h-24 w-24 mx-auto">
                    <svg className="h-24 w-24 -rotate-90" viewBox="0 0 36 36">
                      ...
                      <circle
                        ...
                        strokeLinecap="round"
                        className={Number(score) >= 80 ? "text-emerald-500" : Number(score) >= 60 ? "text-primary" : "text-amber-500"}
                      />
                    </svg>
                    <span className={`absolute inset-0 flex items-center justify-center text-2xl font-bold ${Number(score) >= 80 ? "text-emerald-500" : "text-primary"}`}>
                      {score}%
                    </span>
                  </div>
