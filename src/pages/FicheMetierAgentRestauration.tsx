...
import heroImg from "@/assets/metier-agent-restauration.jpg?format=webp";
...
const MISSIONS = [
  "Préparer les ingrédients et assembler les plats simples (sandwichs, salades, plats chauds)",
  "Assurer le service au comptoir ou en libre-service",
  "Maintenir la propreté des espaces de travail et de la salle",
  "Réapprovisionner les vitrines et les présentoirs alimentaires",
  "Respecter les normes d'hygiène HACCP et la chaîne du froid",
  "Encaisser les clients avec rapidité et courtoisie",
];
const COMPETENCES = [
  { icon: UtensilsCrossed, text: "Préparation rapide et assemblage alimentaire" },
  { icon: ShieldCheck, text: "Normes HACCP et hygiène alimentaire" },
  { icon: GraduationCap, text: "Polyvalence et rapidité d'exécution" },
  { icon: Wrench, text: "Utilisation des équipements de restauration rapide" },
];
const CONDITIONS = [
  "Horaires flexibles avec possibilité de temps partiel ou coupures",
  "Travail debout dans un environnement dynamique et cadencé",
  "Accessible sans diplôme, formation rapide sur le terrain",
  "Évolution possible vers responsable d'équipe ou cuisinier",
];
...
export default function FicheMetierAgentRestauration() {
  return (
    <>
      <Helmet>
        <title>Agent polyvalent de restauration – Code ROME G1501 | AXIOM Talents</title>
        <meta name="description" content="Fiche métier Agent polyvalent de restauration (ROME G1501) : missions, compétences, salaire en France et accompagnement AXIOM & ALTIS." />
        <link rel="canonical" href="https://axiom-talents.com/fiches-metiers/g1501-agent-polyvalent-restauration" />
        <script type="application/ld+json">{JSON.stringify({
          "@context": "https://schema.org", "@type": "JobPosting",
          "title": "Agent polyvalent de restauration", "description": "Préparer et assembler les plats, assurer le service et le nettoyage en restauration rapide ou collective. Métier en haute tension en France.",
          "identifier": { "@type": "PropertyValue", "name": "ROME", "value": "G1501" },
          ...
          "baseSalary": { "@type": "MonetaryAmount", "currency": "EUR", "value": { "@type": "QuantitativeValue", "value": 1800, "minValue": 1650, "maxValue": 2100, "unitText": "MONTH" } },
          "industry": "Restauration collective / rapide", "occupationalCategory": "G1501"
        })}</script>
      </Helmet>

      <div className="min-h-screen bg-background text-foreground">
        <header className="sticky top-0 z-40 border-b bg-background/80 backdrop-blur-md"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3"><Link to="/" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"><ArrowLeft className="h-4 w-4" /> Retour à l'accueil</Link><Badge variant="outline" className="border-accent/40 text-accent text-xs font-mono">ROME G1501</Badge></div></header>

        <motion.section initial="hidden" animate="visible" className="relative overflow-hidden">
          <div className="absolute inset-0"><img src={heroImg} alt="Agent polyvalent de restauration" className="h-full w-full object-cover opacity-20" loading="eager" /><div className="absolute inset-0 bg-gradient-to-b from-background/60 via-background/90 to-background" /></div>
          <div className="relative mx-auto max-w-6xl px-4 py-20 md:py-28">
            ...
            <motion.h1 custom={1} variants={fadeUp} className="font-display text-3xl font-bold md:text-5xl">Agent polyvalent de restauration</motion.h1>
            <motion.p custom={2} variants={fadeUp} className="mt-2 text-muted-foreground text-lg">Code ROME G1501 · Restauration collective & rapide</motion.p>
            <motion.div custom={3} variants={fadeUp} className="mt-6 flex items-center gap-6 flex-wrap">
              <span className="flex items-center gap-2 text-success font-semibold"><Euro className="h-5 w-5" /> 1 650 – 2 100 € net/mois</span>
        ...
        <MotivationalQuote quote="Votre énergie et votre polyvalence sont des qualités essentielles dans la restauration française." />
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 grid gap-10 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            <Card className="border-accent/30 bg-accent/5"><CardContent className="p-6 space-y-4">
              ...
              <Button asChild className="w-full bg-accent text-accent-foreground hover:bg-accent/90"><Link to="/signup-light?rome=G1501">Vérifier mon éligibilité et commencer mon parcours <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </CardContent></Card>
            <Card><CardContent className="p-6 space-y-3"><h3 className="font-bold">En résumé</h3><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-accent text-lg">G1501</p><p className="text-muted-foreground text-xs">Code ROME</p></div><div className="rounded-lg bg-muted p-3 text-center"><p className="font-bold text-success text-lg">~1 875 €</p><p className="text-muted-foreground text-xs">Salaire moyen</p></div></div></CardContent></Card>
          </div>
        </div>
