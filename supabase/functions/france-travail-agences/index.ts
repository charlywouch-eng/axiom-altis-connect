...
const AGENCES_URL = "https://api.francetravail.io/partenaire/referentielagences/v1/agences";

async function getToken(clientId: string, clientSecret: string): Promise<string | null> {
  const scopes = [
    "api_referentielagencesv1 o2dsoffre",
  ...
  try {
    ...
    if (token) {
      ...
      if (res.ok) {
        ...
        if (agences.length > 0) {
          console.log(`Agences API returned ${agences.length} results`);
          return new Response(JSON.stringify({ agences, source: "api", total: rawAgences.length }), {
            status: 200,
        ...
        const text = await res.text();
        console.log(`Agences API error [${res.status}]: ${text.substring(0, 200)}`);
      }
    }
